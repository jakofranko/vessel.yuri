...
class Entity
  ...
  def generate_children
    ...
    num_children.times do
      ...
      options[:name] = if options[:name] == false || options[:name_self] == true
                         type.to_s
                       else
                         @LANG.make_name(type)
                       end
  ...
  def describe
    ...
    sentence << if @NAME != @TYPE
                  "#{@NAME}, #{article} #{adj} #{@TYPE.downcase}"
                else
                  "#{article.capitalize} #{adj} #{@TYPE.downcase}"
                end

    if @VERB && !@VERB.empty?
      sentence << if @ADVB && !@ADVB.empty?
                    ", #{@ADVB} #{@verb}"
                  else
                    ", #{@VERB}"
                  end
  ...
  def validate(type, options)
    ...
    raise "Please specify the entity's type" if type['TYPE'].nil?

    ##
    # If the entity has children, make sure their descriptors are set
    return unless !type['CMAX'].nil? && type['CMAX'].to_i.positive?
    if type['CTPS'].nil?
